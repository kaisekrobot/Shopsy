...
DEFAULT_CHANNELS = [
    ch.strip() for ch in 
    os.getenv("DEFAULT_CHANNELS", "@your_channel_1,@your_channel_2").split(",")
    if ch.strip()
]
...
(MAIN_MENU, PLAY_GAME, PROFILE, STATS, HELP, REFERRAL,
 ...
 SHOPSY_LOGIN, SHOPSY_OTP) = range(17)
...
"""
    ...
    async def handle_admin_unlimited(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
        ...
        await query.edit_message_text(
            "♾️ **Unlimited Mode Enabled**\n\n"
            "You now have unlimited access to all features.\n"
            "This mode will persist until disabled.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 Back", callback_data="admin_panel")],
            ]),
            parse_mode=ParseMode.MARKDOWN

...
- OTP Login Flow Fixed
- Admin Gift Points System Added
...
class AsyncShopsyClient:
        ...
        *,
        ...
        for attempt in range(5):
            ...
            try:
                ...
                self.log.warn(f"Request attempt {attempt + 1} failed: {e}")
                if attempt == 4:
    ...
    async def send_otp(self, phone: str) -> str:
        ...
        if not data.get("RESPONSE", {}).get("actionSuccess"):
            error_msg = data.get("ERROR_MESSAGE") or "OTP send failed"
            self.log.warn(f"OTP send failed: {error_msg}")
            raise RuntimeError(f"OTP send failed: {error_msg}")
        ...
        self.log.ok(f"OTP sent successfully | requestId={request_id[:12]}...")
        return request_id
    ...
    async def verify_otp(self, phone: str, otp: str, otp_request_id: str) -> None:
        ...
        if not response_ctx.get("authenticationSuccess"):
            error_msg = data.get("ERROR_MESSAGE") or "Login failed"
            self.log.warn(f"OTP verification failed: {error_msg}")
    ...
    def is_valid_session(self) -> bool:
        ...
        if time.time() - self.ctx.last_verified > 604800:
            return False
        return True
    ...
    async def validate_and_refresh(self) -> bool:
        ...
        try:
            ...
            self.log.warn(f"Session validation failed: {e}")
            return False
...
class Database:
    ...
    def create_user(self, user_id: int, username: str, first_name: str, last_name: str = "") -> bool:
        with self.get_connection() as conn:
            ...
            try:
                ...
                conn.commit()
                logging.info(f"✅ Created user {user_id} ({username})")
                return True
    ...
    def award_initial_points(self, user_id: int) -> bool:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"🎁 Awarded {INITIAL_POINTS} initial points to user {user_id}")
    ...
    def add_points(self, user_id: int, points: int) -> bool:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"➕ Added {points} points to user {user_id}")
            return cursor.rowcount > 0
    ...
    def deduct_point(self, user_id: int) -> bool:
        with self.get_connection() as conn:
            ...
            if cursor.rowcount > 0:
                logging.info(f"➖ Deducted 1 point from user {user_id}")
            return cursor.rowcount > 0
    ...
    def add_game_played(self, user_id: int) -> None:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"🎮 Recorded game play for user {user_id}")
    ...
    def add_referral(self, referrer_id: int, referred_id: int) -> bool:
        with self.get_connection() as conn:
            ...
            if row and row[0] is not None:
                logging.warning(f"User {referred_id} already has referrer {row[0]}")
                return False
            ...
            conn.commit()
            logging.info(f"👥 Referral recorded: {referrer_id} -> {referred_id}")
            return True
    ...
    def unban_user(self, user_id: int) -> bool:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"✅ Unbanned user {user_id}")
            return cursor.rowcount > 0
    ...
    def reset_user_points(self, user_id: int) -> bool:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"🔄 Reset points for user {user_id}")
            return cursor.rowcount > 0
    ...
    def save_session_data(self, user_id: int, session_data: str) -> None:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"💾 Saved session data for user {user_id}")
    ...
    def add_channel(self, channel_username: str, added_by: int) -> bool:
        with self.get_connection() as conn:
            ...
            try:
                ...
                conn.commit()
                logging.info(f"📢 Added channel {channel_username}")
                return True
            except sqlite3.IntegrityError:
                return False
    ...
    def remove_channel(self, channel_username: str) -> bool:
        with self.get_connection() as conn:
            ...
            conn.commit()
            if cursor.rowcount > 0:
                logging.info(f"📢 Removed channel {channel_username}")
            return cursor.rowcount > 0
    ...
    def set_setting(self, key: str, value: str) -> None:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"⚙️ Set setting {key} = {value}")
    ...
    def set_queue_setting(self, key: str, value: str) -> None:
        with self.get_connection() as conn:
            ...
            conn.commit()
            logging.info(f"⚙️ Set queue setting {key} = {value}")
...
class GameQueueManager:
    ...
    async def add_to_queue(self, user_id: int, chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
        async with self.lock:
            ...
            logging.info(f"📥 User {user_id} added to queue (position {position})")
            return True
    ...
    async def remove_from_queue(self, user_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
        async with self.lock:
            ...
            logging.info(f"📤 User {user_id} removed from queue")
            return True
...
"""
        ...
        try:
            while True:
                ...
                if active_count < max_concurrent and len(self.waiting_queue) > 0:
                    ...
                    logging.info(f"🎮 Starting game for user {user_id}")
        ...
        if session_data:
            try:
                ...
                if client.is_valid_session():
                    ...
                    logging.warning(f"⚠️ Session expired for user {user_id}")
            except Exception as e:
    ...
    async def _save_shopsy_session(self, user_id: int, client: AsyncShopsyClient):
        try:
            ...
            logging.info(f"💾 Saved session for user {user_id}")
        except Exception as e:
    ...
    async def _validate_and_refresh_session(self, user_id: int, client: AsyncShopsyClient) -> bool:
        try:
            if not client.is_valid_session():
                return False
            async with client:
                result = await client.validate_and_refresh()
                if result:
...
"""
                    ...
                    logging.info(f"✅ Game completed for user {user_id}: {game['name']}")
                else:
                    if point_deducted:
                    ...
                    logging.warning(f"⚠️ Game failed for user {user_id}")
    ...
    async def set_max_concurrent(self, limit: int) -> bool:
        ...
        logging.info(f"⚙️ Set max concurrent to {limit}")
        return True
    ...
    async def toggle_pause(self):
        ...
        logging.info(f"⏸️ Queue pause toggled: {new_state}")
        ...
        for user in all_users:
            ...
            if session_data:
                try:
                    ...
                    logging.error(f"Failed to restore session for {user_id}: {e}")
        logging.info(f"✅ Restored {restored_count} Shopsy sessions")
...
(MAIN_MENU, PLAY_GAME, PROFILE, STATS, HELP, REFERRAL,
 ...
 SHOPSY_LOGIN, SHOPSY_OTP, QUEUE_SET_LIMIT_STATE,
 GIFT_POINTS) = range(19)
...
def get_admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎁 Gift Points", callback_data="admin_gift_points")],
...
def get_referral_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    share_text = f"🎮 Join me on SPIDER X SHOPSY!\n\nEarn points by playing games!\n\nhttps://t.me/{bot_username}?start=ref_{user_id}"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📨 Copy Referral Link", callback_data=f"copy_ref_{user_id}")],
        [InlineKeyboardButton("📤 Share on Telegram", url=f"tg://msg?text={share_text}")],
        [InlineKeyboardButton("🏠 Dashboard", callback_data="back_to_dashboard")],
    ])
...
class SpiderShopsyBot:
    ...
    def setup(self):
        ...
        conv_handler = ConversationHandler(
            entry_points=[
                ...
                CallbackQueryHandler(self.admin_gift_points_start, pattern="admin_gift_points"),
            ...
            states={
                ...
                GIFT_POINTS: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_gift_points)],
    ...
    async def handle_shopsy_phone(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logging.info(f"✅ OTP sent to user {user_id} for phone +91{phone}")
            return SHOPSY_OTP
            
        except Exception as e:
            error_msg = str(e)
            logging.error(f"❌ OTP send failed for user {user_id}: {error_msg}")
            await update.message.reply_text(
    ...
    async def handle_shopsy_otp(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if client.is_valid_session():
                ...
                logging.info(f"✅ User {user_id} logged in to Shopsy successfully")
            ...
            error_msg = str(e)
            logging.error(f"❌ OTP verification failed for user {user_id}: {error_msg}")
            await update.message.reply_text(
                f"❌ **Login failed**\n\n"
                f"Error: {error_msg[:200]}\n\n"
                "Please try again with /login",
    ...
    async def handle_referral(self, user_id: int, referrer_id: int):
        user_data = db.get_user(user_id)
        if not user_data:
            logging.warning(f"User {user_id} not found for referral")
            return
        
        if user_data.get('referral_by') is not None:
            logging.info(f"User {user_id} already has referrer {user_data.get('referral_by')}")
            return
        ...
        if success:
            try:
                ...
                await self.application.bot.send_message(
                    ...
                    f"Total Referrals: {db.get_user_stats(referrer_id).get('referrals', 0)}",
                    parse_mode=ParseMode.MARKDOWN
                )
                logging.info(f"👥 Referral reward sent to {referrer_id}")
            except Exception as e:
            ...
            try:
                ...
                await self.application.bot.send_message(
                    user_id,
                    f"✅ **Referral Applied Successfully!**\n\n"
                    f"You were referred by an existing user.\n"
                    f"🎮 Enjoy your bonus points!",
                    parse_mode=ParseMode.MARKDOWN
                )
            except Exception as e:
                logging.error(f"Failed to notify referred user {user_id}: {e}")
...
"""
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data.startswith("copy_ref_"):
            try:
                ref_user_id = int(data.replace("copy_ref_", ""))
                bot_username = BOT_USERNAME or os.getenv("BOT_USERNAME", "YourBotUsername")
                ...
                await query.edit_message_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("📤 Share on Telegram", url=f"tg://msg?text={ref_link}")],
                        [InlineKeyboardButton("🏠 Dashboard", callback_data="back_to_dashboard")],
                    ]),
                    parse_mode=ParseMode.MARKDOWN
                )
            except Exception as e:
                logging.error(f"Copy referral error: {e}")
            return
        ...
        if data == "check_joined":
            ...
            await self.admin_gift_points_start(update, context)
...
"""
    ...
    # ============================================================
    #  ADMIN GIFT POINTS SYSTEM
    # ============================================================
    
    async def admin_gift_points_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.callback_query.from_user.id not in ADMIN_IDS:
            return
    ...
    async def handle_gift_points(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if success:
                # Notify admin
                await update.message.reply_text(
                    f"✅ **Successfully gifted {points} points**\n\n"
                    f"👤 User: @{user_data.get('username', 'Unknown')} (`{target_id}`)\n"
                    f"💎 New Balance: {db.get_user_stats(target_id).get('points', 0)} points",
                    parse_mode=ParseMode.MARKDOWN
                )
                logging.info(f"🎁 Admin {admin_id} gifted {points} points to user {target_id}")
                
                # Notify user
                try:
                    await context.bot.send_message(
                        target_id,
                        f"🎉 **Admin has gifted you {points} points!**\n\n"
                        f"💎 Your new balance: {db.get_user_stats(target_id).get('points', 0)} points\n"
                        f"🎮 You can now play more games!",
                        parse_mode=ParseMode.MARKDOWN
                    )
                    logging.info(f"✅ Gift notification sent to user {target_id}")
                except Exception as e:
                    logging.error(f"Failed to notify user {target_id}: {e}")
            else:
                await update.message.reply_text(
                    f"❌ Failed to gift points to user `{target_id}`.",
                    parse_mode=ParseMode.MARKDOWN
                )
    ...
    def run(self):
        ...
        # Add graceful shutdown handlers for Railway
